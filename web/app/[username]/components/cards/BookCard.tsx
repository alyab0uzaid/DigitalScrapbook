...
// ── 2x1 ──────────────────────────────────────────────────────────────────────
// Left half: cover centered horizontally, bleeds bottom edge. Right half: text bottom, left-aligned.
export function BookCard2x1({ item }: { item: Item }) {
  const author = item.metadata?.author as string | null
  return (
    <div className="absolute inset-0 flex">
      {/* Left half — true 50% of card, overflow-hidden clips the bottom bleed */}
      <div className="w-1/2 h-full relative overflow-hidden">
        {item.image_url && (
          <div
            className="absolute left-0 right-0 flex justify-center transition-transform duration-300 ease-out group-hover:-translate-y-3"
            style={{ bottom: '-60px', filter: coverShadow }}
          >
            <div style={{ clipPath: 'inset(0 round 3px)' }}>
              <Image
                ...
                height={0}
                sizes="300px"
                style={{ height: 220, width: 'auto', maxWidth: 180, display: 'block' }}
              />
            </div>
          </div>
        )}
      ...
      <div className="w-1/2 h-full flex flex-col items-start justify-end gap-1.5 pb-4 pl-4 pr-4">
        <StatusPill status={item.status} />
